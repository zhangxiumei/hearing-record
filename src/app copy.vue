<template>
    <div class="clearfix page-content">
        <div class="page-left">
            <hearingRecord :list="list"></hearingRecord>
            <div class="contenteditable-wrap" :style="{ background: 'url(' + linebg + ') repeat' }">
                ...
                <div class="contenteditable" contenteditable="true" v-for="(item, index) in filterList"
                    ...
                    :class="{ quesition: item.type == 1, answer: item.type == 2 }"
                    :style="{ lineHeight: lineHeight + 'px' }" @keydown="addItem($event, index)"
                    :ref="'contenteditable' + index" :index="index" :key="item.key">
        ...
        <div class="page-right">
            <div class="current-quesition" v-if="currentIndex !== -1 && currentIndex < quesitionList.length">
                <div :style="{ background: 'url(' + linebg + ') repeat' }">
                    <p :style="{ lineHeight: lineHeight + 'px' }">问：{{ quesitionList[currentIndex][0] }}</p>
                    <p :style="{ lineHeight: lineHeight + 'px' }">答：{{ quesitionList[currentIndex][1] }}</p>
                </div>
            </div>
            <div class="quesition-lists-wrap">
                <ul class="quesition-lists" :style="{ background: 'url(' + linebg + ') repeat' }">
                    <li v-for="(item, index) in quesitionList" :key="index">
                        <a @click.prevent.stop="showQuesition($event, index)"
                            @dblclick.prevent.stop="addQuesition($event, index)" :style="{ lineHeight: lineHeight + 'px' }"
...
<script>
import hearingRecord from '@/components/hearingRecord/hearing-record.vue'
console.log(hearingRecord)
export default {
    name: 'App',
    components: { hearingRecord },
    data: function () {
        return {
            list: ['问：第一问', '答：第一答'],
            quesitionList: [['你是否人大代表或政协委员', '不是'], ['根据法律规定，我们请通晓手语的人***到场，请他提供帮助，你明白了吗（通过手语）？', '（通过手语）我明白了']],
            linebg: '',
            lineHeight: 30,
            currentIndex: -1
        }
